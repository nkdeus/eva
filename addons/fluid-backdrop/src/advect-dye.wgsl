...
fn main(@builtin(global_invocation_id) id: vec3u) {
  ...
  var color = input.dye_dissipation * sample_dye(backtrace);

  let gain = input.emitter_gain;
  color += input.color_a * emitter_weight(p, input.idle_a, aspect) * 0.12 * gain;
  color += input.color_b * emitter_weight(p, input.idle_b, aspect) * 0.115 * gain;
  if (input.pointer_active > 0.0) {
    let weight = segment_weight(p, input.pointer_from, input.pointer_to, input.splat_radius, aspect);
    color += input.pointer_color * weight * input.ink;
  }
